	...
	<div class="space-y-4">
		<div class="flex items-center justify-between">
			<span class="text-sm font-bold text-slate-700 dark:text-slate-300"
				>Simulate Operating System Theme:</span
			>
			<button
		...
		<div
			class="flex items-center justify-center rounded-2xl border border-slate-200 p-8 transition-colors {isDarkMode
				? 'bg-slate-950 text-white'
				: 'bg-slate-100 text-slate-900'}"
		>
			<div
				class="flex items-center gap-4 rounded-xl border border-slate-300/40 bg-white/10 px-5 py-3 backdrop-blur-md"
			>
				...
				<div>
					...
					<p class="text-sm opacity-75">
						Favicon fill: {isDarkMode ? '#818cf8 (Indigo 400)' : '#4f46e5 (Indigo 600)'}
					</p>
				</div>
			</div>
		</div>
...
<LabCard title="CSS Mask Icon vs Inline SVG" badge="1 DOM Node">
	<div class="space-y-4 font-mono text-sm">
		<div class="grid grid-cols-2 gap-4">
			<div>
				...
				<input
					...
					class="h-10 w-full cursor-pointer rounded-xl border border-slate-300"
				/>
			</div>
			<div>
				<label for="icon-size-range" class="mb-1 block font-bold text-slate-600 dark:text-slate-400"
					>Icon Size: {iconSize}px</label
				>
				<input
					id="icon-size-range"
					type="range"
		...
		<div
			class="flex items-center justify-around rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-800 dark:bg-slate-950"
		>
			<div class="space-y-2 text-center">
				...
				></div>
				<p class="text-sm font-bold text-slate-500 uppercase">CSS Mask (1 DOM Node)</p>
			</div>
		</div>
	</div>
</LabCard>
