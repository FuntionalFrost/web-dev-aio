...
<LabShell codeHtml={data.codeHtml} rawCode={data.rawCode} filename={data.filename}>
	{#snippet guide()}
		...
		<p class="text-base sm:text-lg">
			Svelte 5 replaces legacy stores and component-bound reactivity with fine-grained signal runes
			that execute uniformly across components, classes, and standalone TypeScript modules.
		</p>
		<ul>
			<li>
				<strong>Universal Runes:</strong> <code>$state</code>, <code>$derived</code>, and
				<code>$effect</code>
				work anywhere—including TypeScript classes inside <code>.svelte.ts</code> files.
			</li>
			<li>
				<strong>Snippets (<code>&#123;#snippet&#125;</code> & <code>@render</code>):</strong> Replaces
				fragile slot mechanisms with strongly typed, parameterizable markup blocks.
			</li>
			<li>
				<strong>Explicit Prop Bindings (<code>$props()</code> & <code>$bindable()</code>):</strong> Declares
				whether a component property allows two-way parent binding.
			</li>
		</ul>
	{/snippet}
			...
			<div class="space-y-4 font-mono text-sm">
				<div class="grid grid-cols-2 gap-3">
					<div>
						<label
							for="item-name-input"
							class="mb-1 block text-sm font-bold text-slate-500 uppercase">Item Name:</label
						>
						<input
							id="item-name-input"
							type="text"
					...
					<div>
						<label
							for="item-price-input"
							class="mb-1 block text-sm font-bold text-slate-500 uppercase">Price ($):</label
						>
				...
				<div>
					<label
						for="discount-range-input"
						class="mb-1 block text-sm font-bold text-slate-500 uppercase"
					>
				...
				<div class="grid grid-cols-3 gap-3">
					...
					>
						<span class="text-sm font-bold text-slate-500 uppercase">Items Count</span>
					...
					<div
						class="rounded-xl border border-slate-200 bg-slate-50 p-3.5 text-center dark:border-slate-800 dark:bg-slate-950"
					>
						<span class="text-sm font-bold text-slate-500 uppercase">Subtotal</span>
						<p class="mt-1 text-2xl font-bold text-slate-700 dark:text-slate-300">
							${store.subtotal}
						</p>
					</div>
					<div
						class="rounded-xl border border-emerald-500/30 bg-emerald-50/40 p-3.5 text-center dark:bg-emerald-950/30"
					>
						...
						>
							...
							<div class="flex items-center gap-3">
								...
								<button
									...
									class="rounded-lg bg-rose-50 px-2 py-1 text-sm font-bold text-rose-600 hover:bg-rose-100 dark:bg-rose-950/50 dark:text-rose-400"
								>
									Remove
								</button>
							</div>
