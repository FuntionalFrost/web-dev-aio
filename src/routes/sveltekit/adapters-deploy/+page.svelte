<script lang="ts">
	...
	> = {
		...
		vercel: {
			...
			idealFor:
				'Vercel platform features (Incremental Static Regeneration ISR, analytics, previews)',
			configSample:
...
<LabShell codeHtml={data.codeHtml} rawCode={data.rawCode} filename={data.filename}>
	{#snippet guide()}
		...
		<p class="text-base sm:text-lg">
			SvelteKit's adapter architecture compiles your universal codebase for any deployment target
			without changing application source code.
		</p>
		<ul>
			<li>
				<strong><code>@sveltejs/adapter-static</code>:</strong> Pre-renders entire static sites for zero-server
				CDN hosting with zero compute runtime costs.
			</li>
			<li>
				<strong><code>@sveltejs/adapter-cloudflare</code>:</strong> Compiles dynamic server routes into
			...
			<li>
				<strong><code>@sveltejs/adapter-vercel</code>:</strong> Automatically maps server routes to Vercel
				Serverless/Edge functions with ISR support.
			</li>
		</ul>
	{/snippet}
		...
		<LabCard title="SvelteKit Adapter Selector & Config Generator" badge="Adapter Topologies">
			<div class="space-y-4 font-mono text-sm">
				<div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
					{#each ['static', 'cloudflare', 'node', 'vercel', 'auto'] as const as ad (ad)}
						<button
							onclick={() => (selectedAdapter = ad)}
							class="rounded-xl border p-2.5 text-center text-sm font-bold transition {selectedAdapter ===
							ad
				...
				>
					<div>
						<span class="text-sm font-bold text-slate-500 uppercase">NPM Package:</span>
						<p class="text-base font-bold text-indigo-600 dark:text-indigo-400">
							{adapterSpecs[selectedAdapter].package}
						</p>
					</div>
					<div>
						<span class="text-sm font-bold text-slate-500 uppercase">Build Output:</span>
						<p class="text-sm font-semibold text-slate-900 dark:text-white">
							{adapterSpecs[selectedAdapter].output}
						</p>
					</div>
					<div>
						<span class="text-sm font-bold text-slate-500 uppercase">Ideal Deployment Target:</span>
						<p class="text-sm text-slate-700 dark:text-slate-300">
							{adapterSpecs[selectedAdapter].idealFor}
						</p>
					</div>

					<div class="overflow-x-auto rounded-xl bg-slate-900 p-3.5 text-sm text-slate-300">
						<span class="mb-1 block font-bold text-slate-500">svelte.config.js snippet:</span>
						<pre class="font-mono text-sm text-emerald-400 sm:text-sm">{adapterSpecs[
								selectedAdapter
							].configSample}</pre>
					</div>
				</div>
