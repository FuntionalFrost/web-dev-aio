...
<LabShell
	moduleId="infra-databases-orm"
	title={data.meta.title}
	description={data.meta.description}
>
	{#snippet guide()}
		<div class="not-prose">
			<CodeBlock codeHtml={data.codeHtml} rawCode={data.rawCode} filename="schema-and-db.ts" />
		</div>
		...
		<ul>
			...
			<li>
				<strong>The Serverless Connection Exhaustion Problem:</strong> Each serverless or edge function
				instance opens its own TCP connection. Without proxy poolers (PgBouncer, Neon WebSockets, AWS
				RDS Proxy), database connection limits are rapidly overwhelmed.
			</li>
			<li>
		...
		<div class="space-y-6">
			...
			>
				...
				>
					...
					>
						Type-Safe Query Builder Simulator
					</span>
					<span class="font-mono text-[10px] text-slate-500">Drizzle AST → SQL</span>
				</div>
				...
				<div class="grid grid-cols-3 gap-3 font-mono text-xs text-slate-600 dark:text-slate-400">
					<div>
						...
						<select
							...
							class="w-full rounded-lg border border-slate-300 bg-slate-50 px-2.5 py-1.5 text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
						>
			...
			<!-- Serverless Connection Pool Telemetry -->
			<div
				class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50"
			>
				<div
				...
				>
					...
					<div class="flex gap-2">
						<button
							...
							class="rounded-lg bg-indigo-600 px-3 py-1 font-mono text-xs font-semibold text-white transition hover:bg-indigo-500"
						>
							+ Traffic Spike
						</button>
						<button
