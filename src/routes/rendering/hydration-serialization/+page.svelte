<script lang="ts">
	...
	const serializationOutputs = {
		json: {
			encoded:
				'{"timestamp":"2026-09-10T10:00:00.000Z","bigIntError":"TypeError: Do not know how to serialize a BigInt"}',
...
<LabShell codeHtml={data.codeHtml} rawCode={data.rawCode} filename={data.filename}>
	{#snippet guide()}
		...
		<p class="text-base sm:text-lg">
			Passing state across the server-to-client boundary requires specialized serializers like <code
				>devalue</code
			>
		...
		<ul>
			...
			<li>
				<strong><code>devalue</code> Engine:</strong> Powers SvelteKit and Nuxt payload transfers by serializing
			...
			<div class="space-y-4 font-mono text-sm">
				<div class="grid grid-cols-2 gap-3">
					<button
						...
						class="rounded-xl border p-3 text-center text-sm font-bold transition {serializationMode ===
						'devalue'
				...
				>
					<div>
						<span class="text-sm font-bold text-slate-500 uppercase">Serialized Wire Format:</span>
						<div class="mt-1 overflow-x-auto rounded-xl bg-slate-900 p-3.5 text-sm text-slate-100">
							<pre class="font-mono text-sm text-emerald-400">{serializationOutputs[
									serializationMode
								].encoded}</pre>
						</div>
					</div>
					...
					<div>
						...
						<p class="mt-0.5 text-sm font-semibold text-slate-900 dark:text-white">
							{serializationOutputs[serializationMode].loss}
						</p>
					</div>
					...
					<div class="border-t border-slate-200 pt-2 dark:border-slate-800">
						...
						<p class="mt-0.5 text-sm font-bold text-slate-900 dark:text-white">
							{serializationOutputs[serializationMode].safety}
						</p>
					</div>
				</div>
			</div>
